...
const GF256_INVERSE: [u8; 256] = [
    ...
    224, 62, 76, 102, 144, 222, 85, 128, 160, 131, 75, 42, 108, 237, 57, 81, 96, 86, 44, 138, 112,
    208, 31, 74, 38, 139, 51, 110, 72, 137, 111, 46, 164, 195, 64, 94, 80, 34, 207, 169, 171, 12,
    21, 225, 54, 95, 248, 213, 146, 78, 166, 4, 48, 136, 43, 30, 22, 103, 69, 147, 56, 35, 104,
    140, 129, 26, 37, 97, 19, 193, 203, 99, 151, 14, 55, 65, 36, 87, 202, 91, 185, 196, 23, 77, 82,
    141, 239, 179, 32, 236, 47, 50, 40, 209, 17, 217, 233, 251, 218, 121, 219, 119, 6, 187, 132,
    205, 254, 252, 27, 84, 161, 29, 124, 204, 228, 176, 73, 49, 39, 45, 83, 105, 2, 245, 24, 223,
    68, 79, 155, 188, 15, 92, 11, 220, 189, 148, 172, 9, 199, 162, 28, 130, 159, 198, 52, 194, 70,
    5, 206, 59, 13, 60, 156, 8, 190, 183, 135, 229, 238, 107, 235, 242, 191, 175, 197, 100, 7, 123,
    149, 154, 174, 182, 18, 89, 165, 53, 101, 184, 163, 158, 210, 247, 98, 90, 133, 125, 168, 58,
    41, 113, 200, 246, 249, 67, 215, 214, 16, 115, 118, 120, 153, 10, 25, 145, 20, 63, 230, 240,
    134, 177, 226, 241, 250, 116, 243, 180, 109, 33, 178, 106, 227, 231, 181, 234, 3, 143, 211,
    201, 66, 212, 232, 117, 127, 255, 126, 253,
];

// static INIT_INV_TABLE: Once = Once::new();
// static  mut GF_256_INV: [u8;256] = [0u8;256]
const ZERO_POLY: Poly = Poly {
    degree: 0,
    cof: [0; MAX_DEGREE],
};
...
impl Poly {
    pub fn from(degree: u8, coefficients: &[u8]) -> Poly {
        debug_assert!(
            (degree + 1) as usize == coefficients.len(),
            "degree must match coffecients-len"
        );
    ...
    pub fn mut_add(&mut self, term: Term) {
        if term.degree() > self.degree {
            debug_assert!(
                (term.degree() as usize) < self.cof.len(),
                "len exceeded term.degree={}, len={}",
                term.degree(),
                self.cof.len()
            );
            ...
            self.cof[term_degree as usize] = gf256_add(self.cof[term_degree], term.coef());
            //xor
        }
...
pub mod gf_tests {
    ...
    fn test_term_div() {
        // 4x^2
        assert_eq!(Term(2, 4).div(Term(1, 2)), Term(1, 2));
        assert_eq!(Term(2, 4).div(Term(3, 2)), Term(0, 0));
    }
    ...
    fn test_poly_mul_term() {
        ...
        assert_eq!(&terms, &[Term(6, 24), Term(4, 68), Term(3, 128)]);

        let result = poly.multiply(Term(4, 2)); // same as scalar 64
        assert_eq!(result.degree, 10);
        let terms: Vec<Term> = result.terms().filter(|t| t.coef() > 0).collect();
        assert_eq!(&terms, &[Term(10, 24), Term(8, 68), Term(7, 128)]);
    }
    ...
    fn test_poly_remainder() {
        ...
        assert_eq!(
            &remainder_terms,
            &[Term(3, 55), Term(2, 230), Term(1, 120), Term(0, 217)]
        );
    }
    ...
    fn test_remainder_from_code_words() {
        ...
        let data = Poly::from(
            18,
            &[
                0x41, 0x16, 0x97, 0x36, 0x16, 0x96, 0x16, 0x82, 0xD7, 0x06, 0x57, 0x27, 0x56, 0xD6,
                0x16, 0xC6, 0xC6, 0x13, 0x10,
            ],
        );
        ...
        assert_eq!(
            &bytes,
            &[
                0x40, 0x86, 0x97, 0x36, 0x16, 0x96, 0x16, 0x82, 0xD7, 0x00, 0xEC, 0x11, 0xEC, 0x11,
        ...
        let expected: Vec<u8> = "5C 5A 9A 55 CB 35 7F"
    ...
    fn test_remainder_from_long_code_words_version5() {
        ...
        let expected_rem = hex_str_to_bytes(
            "5E 9C 9F 79 9E B0 51 CC C1 27 EB D4 04 05 19 D9 BB 61 E8 93 76 3E 86 85 08 47",
        );
        let rem = msg_poly.div_remainder(&gen_poly);
        let rem_terms: Vec<u8> = rem.terms().map(|t| t.coef()).collect();
    ...
    fn test_remainder_from_long_code_words_0() {
        ...
        let msg_poly = Poly::from((data.len() - 1) as u8, &data);
        let gen_poly = Poly::from(10, &[1, 216, 194, 159, 111, 199, 94, 95, 113, 157, 193]);
